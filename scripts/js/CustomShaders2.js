...
var GlitchShader4 = function(){
        ...
        this.fragmentShader = [
			...
			"	const float max_ofs_siz = 1.5; //TOOD: input",
			...
			"	float t = 5.0 * trunc( time_s, tf );",
			"	float vt_rnd = 0.5*rand( trunc(uv.xy + t, vec2(11)) );",
			"	vt_rnd += 0.5 * rand(trunc(uv.xy + t, vec2(7)));",
			"	vt_rnd = vt_rnd*2.0 - 1.0;",
			"	vt_rnd = sign(vt_rnd) * sat( ( abs(vt_rnd) - glitch_threshold) / (1.0-glitch_threshold) );",
			...
			// "    sample.r *= rand(vec2(color.r, color.r))*vt_rnd*1.0;//*sat(vt_rnd - yuv_threshold);",
			// "	sample.g *= rand(vec2(color.g, color.g))*vt_rnd*1.0;//*sat(vt_rnd - yuv_threshold);",
			// "	sample.b *= rand(vec2(color.b, color.b))*vt_rnd*1.0;//*sat(vt_rnd - yuv_threshold);",
			// "    sample.rgb = mix(noise.rgb, sample.rgb, 0.5)*vt_rnd*1.0;//*sat(vt_rnd - yuv_threshold);",
			"    sample.rgb = pinLight(noise.rgb, color.rgb)*vt_rnd*0.8;//*sat(vt_rnd - yuv_threshold);",
			"    sample.rgb = max(sample.rgb,color.rgb);",
			"    //color.rgb = pinLight(color.rgb,sample.rgb);",
			"	//vec3 sample_yuv = rgb2yuv( sample.rgb );",
			"	//sample_yuv.y /= 1.0-3.0*abs(vt_rnd);// * sat( yuv_threshold - vt_rnd );",
			"	//sample_yuv.z += 0.125 * vt_rnd;// * sat( vt_rnd - yuv_threshold );",
